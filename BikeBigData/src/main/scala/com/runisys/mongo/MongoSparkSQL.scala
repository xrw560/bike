...
object MongoSparkSQL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val session = SparkSession.builder()
      ...
      .config("spark.mongodb.input.uri", "mongodb://xiaoniu:123456@192.168.170.181:27017/xiaoniu.logs")
      .config("spark.mongodb.output.uri", "mongodb://xiaoniu:123456@192.168.170.181:27017/xiaoniu.result")
      .getOrCreate()
    ...
    val uv = session.sql("select count(1) pv, count(distinct openid) uv,now() as time from v_logs")
//    val uv = session.sql("select count(distinct openid) from v_logs")
